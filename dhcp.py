...
class ApiRos:
    ...
    def updateserver(self, address, number, poolname):
	self.inputSentence = ['/ip/dhcp-server/set']
	self.inputSentence.append('=src-address='+ address)
	self.inputSentence.append('=address-pool='+ poolname)
	self.inputSentence.append('=numbers='+ number)
	self.writeSentence(self.inputSentence)
	self.readSentence()
    def emptyline(self):
...
def main():
	...
	apiros.updateserver(sys.argv[2], sys.argv[3], sys.argv[4]);
    ...
    elif sys.argv[1] == "adddhcp":
	apiros.addpool(sys.argv[2],sys.argv[3]);
	apiros.addnetwork(sys.argv[4],sys.argv[5]);
	apiros.addserver(sys.argv[6], sys.argv[2]);
